...
function updateCountdown() {
    ...
    const countdownEl = document.getElementById('countdown');  // Get the countdown element
    ...
    if (time <= 0) {
        ...
        if (time <= 10) {
            ...
            countdownEl.style.color = ''; // or set it to the original color
        }
    }
}
...
async function loadQuestion() {
    if (askedCount < totalQuestion) {
        ...
        if (data.response_code === 0 && data.results.length > 0) {
            ...
            // Handle the case where no question is received from the API
